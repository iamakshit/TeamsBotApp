import { BotCommand } from "../helpers/botCommand";
import { Utils } from "../helpers/utils";
const rawLearnCard = require("../adaptiveCards/learn.json");
const rawLearnCard2 = require("../adaptiveCards/learn2.json");
const rawLearnCard3 = require("../adaptiveCards/learn3.json");

export class LearnCommand extends BotCommand {
  constructor() {
    super();
    this.matchPatterns = [/^\s*learn\s*/];
  }

  validateParameters(parameters: any): boolean {
    if (!parameters.likeCount) {
      throw new Error(`Command "learn" failed: missing input "likeCount"`);
    }
    if (!parameters.likeCount2) {
      throw new Error(`Command "learn" failed: missing input "likeCount"`);
    }
    if (!parameters.likeCount3) {
      throw new Error(`Command "learn" failed: missing input "likeCount"`);
    }
    return true;
  }

  async run(parameters: any): Promise<any> {
    this.validateParameters(parameters);
    const card = Utils.renderAdaptiveCard(rawLearnCard, parameters.likeCount);
    const card2 = Utils.renderAdaptiveCard(rawLearnCard2, parameters.likeCount2);
    const card3 = Utils.renderAdaptiveCard(rawLearnCard3, parameters.likeCount3);

    return await parameters.context.sendActivity({ attachments: [card,card2,card3] });
  }
}
